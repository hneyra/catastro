...
import kamayuk.catastro.KamayukAplicacion;
import kamayuk.catastro.autorizacion.ComprobadorDeAcceso;
import kamayuk.catastro.contribuyentes.infraestructura.DirectorioHttpDeRentas;
import kamayuk.catastro.esquema.BaseDeDatosDePrueba;
import kamayuk.catastro.nucleo.infraestructura.web.SectorController;
import kamayuk.catastro.parametros.infraestructura.ClienteHttpDeNormativa;
...
/**
 * {@code catastro} arranca. Los dos perfiles, con el artefacto de verdad.
 *
 ...
 *
 * <ul>
 *   <li>Quitandole al modulo {@code kamayuk-catastro-seguridad} el {@code ComprobadorDeAccesoJdbc}
 *       —o su {@code @Component}—: «required a bean of type ComprobadorDeAcceso that could not be
 *       found».
...
@DisplayName("C-7 — catastro arranca, en los dos perfiles")
class ArranqueDeLaAplicacionTest {
    ...
    void elPerfilWebLevanta(org.springframework.context.ApplicationContext contexto) {
        assertThat(contexto.getBeanNamesForType(DirectorioHttpDeRentas.class))
                .as("el padron de `rentas`, que inyecta el mapeador")
                .isNotEmpty();

        assertThat(contexto.getBeanNamesForType(ClienteHttpDeNormativa.class))
                .as("la normativa cacheada, que tambien lo inyecta")
                .isNotEmpty();
        ...
        assertThat(contexto.getBeanNamesForType(SectorController.class))
