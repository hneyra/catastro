...
| Su frontend (`catastro-web`) | **Existe desde #32 el armazón, y desde #33 el módulo Catastro entero.** Vive en `frontend/` —no en `catastro-web/`: es el nombre que ya usan `RUTAS_DE_CODIGO` y el `.dockerignore`—. React 19 sobre Vite 6, sin router, sin react-query y sin UI kit. Porta el marco de `CatastroV6.dc.html` con los **seis** módulos de este backend y sus **16** hojas, y **las cuatro hojas del módulo Catastro** que el artboard dibuja —Panel, Predios maestro-detalle, Territorio y Valores del ejercicio—. Todo sale por `src/api/cliente.ts`; los datos los pone un **proxy que sustituye `fetch`** (ADR-0010) y que se apaga con `VITE_CATASTRO_PROXY_DE_DATOS=false`; `servidas.ts` nace **vacía** y crece hasta las 64. **Ninguna cifra se escribe: `src/datos/` sólo tiene rótulos y lo comprueba `yarn datos`**. **Desde #40 su imagen se publica** —desde **#102** se llama `kamayuk-catastro-interfaz` y no `kamayuk-catastro-web`, que ya era el `Deployment` del **backend**— y el `Dockerfile` la construye con `VITE_CATASTRO_PROXY_DE_DATOS=false`, así que **no lleva dentro el proxy de datos simulados**. **Y desde #102 la despliega el descriptor** —`ConfigMap` + `Deployment` + `Service` + `Middleware` + la `IngressRoute` partida en dos: `PathPrefix(/catastro/api/v1)` al backend y `PathPrefix(/catastro)` a la interfaz, con prioridades **explícitas**— y **autentica**: código de autorización con **PKCE S256** escrito a mano contra `kamayuk-backoffice` (el mismo cliente que `rentas`; el realm **no se toca**), con el token **en memoria** y nunca en `localStorage`, y el `redirect_uri` en la raíz de la APLICACIÓN —`/catastro/`, que es la `base` de Vite— porque la del sitio costó el acceso a producción en `rentas`#71. El emisor lo sirve un `ConfigMap` al arrancar el contenedor y no se hornea en la imagen. Su `nginx.conf` **perdió el `proxy_pass`**: apuntaba a `catastro:8080` —un nombre del `compose.yaml`— y nginx resuelve el upstream **al arrancar**, así que el pod no habría arrancado. **Y desde #47 son NUEVE arneses y no ocho**: `imagen` mira los dos archivos que deciden cómo se sirve —`nginx.conf` y `Dockerfile`—, y los mide **levantando nginx de verdad y preguntando por HTTP**, porque `add_header` no se hereda y una guarda de texto pasa en verde con el defecto puesto. **Desde #49 son diez** —`errores`— y **desde #48, ONCE**: `ejercicios` mide que el desplegable de la barra global sale del **reloj**, y lo mide **moviéndolo**, que es la única forma —los cuatro años que estaban escritos a mano coincidían con el reloj letra por letra, así que una guarda que preguntara «ofrece el año en curso» pasaba en verde con el defecto puesto: medido, verde—. **El ejercicio ya no es un literal** (#48): sale del año en curso y tres hacia atrás, el reloj se lee **una vez** en `App.tsx` y las dos funciones que derivan la lista son puras (`shell/ejercicios.ts`). Sigue siendo una **suposición** —nadie le pregunta al backend qué ejercicios tienen conjunto sellado, y esa lectura es **#51**— que falla ruidosa: elegir uno sin sellar da 404 con `parametroQueFalta`. **Y desde #46 son DOCE**: `ficha` compara lo que la pantalla **dibuja** con el JSON que la pagina recibio, en las cuatro clases de ficha. Con el, el tipo `Ficha` declara los **22** componentes que `FichaResource` publica —eran 17— y la pantalla los dibuja: lo construido, las **obras complementarias SIN IMPORTE y diciendo por que** (el Anexo III de la R.M. 277-2025-VIVIENDA no esta transcrito y `otra_instalacion` no tiene columna de importe), los tres bloques de detalle —cada uno por **su** ruta, porque `/urbana/{cod}` fija `TipoFicha.UNICA` y esa ficha no tiene ninguno— y la pestana **Movimientos**, que es la misma ruta con el `?historico=` que esta capa no ofrecia. **Y desde #102 son TRECE**: `identidad` mide la PUERTA —el `redirect_uri`, el reto S256 y donde NO acaba el token— sobre el paquete construido y servido de verdad, y **sin el emisor de mentira** que los otros nueve ponen delante para poder llegar a las pantallas. Con él llegan dos piezas compartidas que no son arneses: `base.mjs`, que lee de `vite.config.ts` dónde vive la aplicación en vez de repetirlo en ocho literales, y `emisor.mjs`. |
| Sus imágenes en `ghcr.io/hneyra/` | **Son TRES desde #40, y dos de ellas ya existen.** Medido contra el registro el 2026-09-06 para el `sha` de `main`: `kamayuk-catastro` → **200**, `kamayuk-catastro-migrador` → **200**, y `kamayuk-catastro-web` → **403**, que es «no se puede concluir nada» y no «no existe» — nace en el primer `push` a `main` que lleve la matriz nueva. La fila anterior decía que la primera **no existía**, y era falso desde D. **Desde #102 la tercera se llama `kamayuk-catastro-interfaz`**, y con el renombrado las publicadas como `kamayuk-catastro-web` **quedan huérfanas**: la etiqueta nueva no existe en el registro hasta que `publicar-imagenes.yml` corra en `main`, así que un despliegue anterior a ese merge dejaría el pod en `ImagePullBackOff`. **Y desde #102 la interfaz SÍ se despliega**: el descriptor la declara con su `ConfigMap`, su `Deployment`, su `Service` y su mitad de la `IngressRoute`. Lo que queda de #40 AC-3 es que `infrastructure` retire `sgtm-interfaz`, la del monolito |
