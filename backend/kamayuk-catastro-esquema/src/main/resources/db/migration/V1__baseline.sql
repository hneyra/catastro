...
--  ----------------------------------------------------------------------------
--  LO QUE SE LE QUITO AL BASELINE GENERADO, Y POR QUE (P5C)
--  ----------------------------------------------------------------------------
--
--  El generador de ADR-0032 restringe el esquema de `sgtm` a las tablas de este
--  sistema, pero arrastra TODA funcion del esquema original. Aqui sobraban CINCO,
--  y no las encontro una revision: las encontro el escaner de frontera de sistema,
--  porque cuatro de ellas consultan una tabla que en esta base no existe. Es el
--  mismo hallazgo que P5B §9 hizo en el baseline de `normativa`.
--
--  Cuatro se van porque NO SON DE ESTE SISTEMA y aqui no tenian ni un disparador
--  que las usara -codigo muerto que ademas miente sobre donde vive la regla-:
--
--    - conjunto_sellado_es_inmutable()             `normativa` (conjunto_parametros)
--    - detalle_de_conjunto_sellado_es_inmutable()  `normativa` (conjunto_parametros)
--    - valuacion_de_publicacion_sellada_es_inmutable()  `normativa` (parametro_tributario)
--    - declaracion_jurada_estado_es_terminal()     `rentas`
--
--  La QUINTA es distinta y conviene tenerlo escrito: `valuacion_de_conjunto_sellado_es_inmutable()`
--  SI es de este sistema -la usaba el disparador `arancel_de_conjunto_sellado_inmutable`, y el
--  arancel es de catastro (ADR-0017: es municipal y se llavea por via)-, pero su CUERPO consulta
--  `conjunto_parametros`, que se fue a `normativa` en P5B. Una funcion que consulta una tabla
--  inexistente no protege nada: REVIENTA en el primer INSERT. Por eso se van las dos, la funcion
--  y su disparador, exactamente como `V2` de `rentas` hizo con las suyas.
--
--  LA GUARDA NO SE PIERDE: se reconstruye en `V3__guarda_del_arancel.sql` contra la copia
--  local de conjuntos sellados que crea `V2`. Entre `V1` y `V3` no hay ninguna ventana en
--  que se pueda escribir un arancel sin guarda, porque las tres migraciones se aplican en
--  la misma corrida del migrador.
--
--  El archivo pasa de 1056 lineas a 940. El original sigue en
--  `sgtm/docs/40-datos/baselines/catastro/V1__baseline.sql`, que no se toca: es el archivo
--  historico.
