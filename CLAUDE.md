...
| `backend/` — **nueve** módulos | **Existe desde P5C**, y desde #4 con `kamayuk-catastro-urbano`: `kamayuk-catastro-{dominio-compartido, esquema, plataforma, parametros, contribuyentes, nucleo, urbano, seguridad, aplicacion}` |
...
| Su esquema | **Existe.** `V1__baseline.sql` (ADR-0032), `V2` —la copia local de conjuntos sellados—, `V3` —la guarda del arancel—, `V4`, `V5` —el buzón de salida—, **`V6`** —el CUC del SNCP y `frente_predio` (ADR-0036, que cierra D-10)— y **`V7`** —la zonificación urbana: `zonificacion`, `parametro_urbanistico`, `seccion_via` y `habilitacion_urbana` (#4)— |
...
backend/                Gradle. Java 25, Spring Boot 4 cuando llegue el negocio
  ...
  kamayuk-catastro-urbano/      la zonificacion vigente y lo que permite (#4). Publica LA ZONA
