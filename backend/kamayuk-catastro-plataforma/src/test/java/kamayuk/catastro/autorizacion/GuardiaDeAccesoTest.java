package kamayuk.catastro.autorizacion;
...
import kamayuk.catastro.compartido.CiudadanoContext;
import kamayuk.catastro.dominio.DocumentoIdentidad;
...
class GuardiaDeAccesoTest {
    ...
    void sinPrivilegio403() throws Exception {
        ...
        MvcResult resultado = mvc.perform(get("/catastro/api/v1/prueba/consulta")).andReturn();
    ...
    void sinFichaLoDice() throws Exception {
        ...
        // funcionario a pedir un permiso que no le habria servido de nada.
        assertThat(cuerpo).doesNotContain("\"detail\":\"No tiene el privilegio");

        // LO QUE ESTA PRUEBA NO PUEDE AFIRMAR, y conviene decirlo: el `title` del problema SIGUE
        // diciendo «No tiene el privilegio necesario para esta operacion», porque sale del codigo
        // y el codigo es `SIN_PRIVILEGIO`. Un codigo nuevo lo arreglaria, y no se anade aqui a
        // proposito: `CodigoDeError` esta copiado en los cuatro backends y en dos frontends, y
        // `catastro`#41 midio que un cliente que no reconoce un codigo lo degrada al del estado
        // HTTP y **saca la pantalla con el titulo de otro error**. O sea que anadirlo sin tocar
        // los seis sitios cambia un mensaje confuso por uno equivocado. Es #22 (la libreria 2).
        assertThat(cuerpo)
                .as("el titulo sigue viniendo del codigo compartido: se deja dicho, no se finge")
                .contains("\"codigo\":\"SIN_PRIVILEGIO\"");
    }
    ...
    void elPrivilegioDelMetodoGana() throws Exception {
        ...
        mvc.perform(get("/catastro/api/v1/prueba/alta")).andReturn();
    ...
    void sesionPropiaPasaSinComprobarElCatalogo() throws Exception {
        ...
        MvcResult resultado = mvc.perform(get("/catastro/api/v1/prueba/sesion")).andReturn();
    ...
    void ciudadanoPasaSoloDesdeSuCadena() throws Exception {
        ...
        try {
            MvcResult resultado = mvc.perform(get("/catastro/api/v1/portal/prueba")).andReturn();
    ...
    void conSoloLaAlternativaPasa() throws Exception {
        ...
        MvcResult resultado = mvc.perform(get("/catastro/api/v1/prueba/dos-opciones")).andReturn();
    ...
    void laAlternativaNoRelajaElPrivilegio() throws Exception {
        ...
        mvc.perform(get("/catastro/api/v1/prueba/dos-opciones")).andReturn();
    ...
    void sinAccesoDeclaradoSeDeniega() throws Exception {
        ...
        MvcResult resultado = mvc.perform(get("/catastro/api/v1/prueba/sin-declarar")).andReturn();
    ...
    static class ControladorDePrueba {
        ...
        @GetMapping("/catastro/api/v1/prueba/consulta")
        String consultar() {
            return "ok";
        }

        @GetMapping("/catastro/api/v1/prueba/alta")
    ...
    static class ControladorSinDeclarar {
        ...
        @GetMapping("/catastro/api/v1/prueba/sin-declarar")
        String sinDeclarar() {
            return "ok";
        }
    }
    ...
    static class ControladorDeDosOpciones {
        ...
        @GetMapping("/catastro/api/v1/prueba/dos-opciones")
        String consultar() {
            return "ok";
        }
    }
    ...
    static class ControladorDeSesionPropia {
        ...
        @GetMapping("/catastro/api/v1/prueba/sesion")
        String sesion() {
            return "ok";
        }
    }
    ...
    static class ControladorDelCiudadano {
        ...
        @GetMapping("/catastro/api/v1/portal/prueba")
        String situacion() {
            return "ok";
        }
    }
