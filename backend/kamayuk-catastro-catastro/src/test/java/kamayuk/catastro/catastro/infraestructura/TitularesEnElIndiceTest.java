        ...
        try (Connection owner = base.conexion(BaseDeDatosDePrueba.OWNER);
                PreparedStatement sentencia =
                        owner.prepareStatement(
                                "ANALYZE predio, contribuyente_de_prueba, titularidad")) {
            sentencia.execute();
            owner.commit();
        }
        sembrado = true;
        ...
        try (Connection app = base.conexion(BaseDeDatosDePrueba.APP)) {
            ...
            ejecutar(
                    ...
                    "INSERT INTO titularidad (municipalidad_id, predio_id, contribuyente_id,"
                            ...
                            // El filtro por municipalidad va ESCRITO, y en produccion no
                            // haria falta: lo pone RLS. `contribuyente_de_prueba` no la lleva
                            // -su `municipalidad_id` es anulable a proposito, ver
                            // EscenarioDelPadron-, asi que sin esta linea la segunda
                            // municipalidad casa tambien con los contribuyentes de la primera y
                            // el predio sale con DOS titulares al 100 %. Es el primero de los
                            // dos defectos que P5B §11 documenta para el escenario de normativa,
                            // repetido aqui por el mismo motivo.
                            + "   JOIN contribuyente_de_prueba c"
                            + "     ON c.codigo_contribuyente = lpad(g::text, 11, '0')"
                            + "    AND c.municipalidad_id = ?",
                    municipalidadId,
                    predios,
                    municipalidadId);
            app.commit();
