...
/**
 * El token con el que el consumidor le pide el buzon a {@code identidad} (#21 AC-2, ADR-0028 §2).
 *
 * <h2>Por que `client_credentials` y no el token de nadie</h2>
 *
 * <p>Al consumidor lo despierta un {@code CronJob}, no una peticion: no hay ningun {@code
 * Authorization} del que tirar. Lo que hay es un cliente confidencial suyo —{@code
 * kamayuk-catastro-servicio-<ubigeo>}, uno por municipalidad— cuya cuenta de servicio lleva el
 * atributo {@code municipalidad_id}, y de ahi sale el claim que acota la lectura del buzon. Y del
 * {@code azp} de ese token sale, del otro lado, QUIEN acusa: {@code identidad} lleva un acuse por
 * consumidor (etapa 3).
 *
 ...
 * multiplicaria los viajes al emisor. Se conserva hasta {@link #MARGEN} antes de su vencimiento; el
 * margen existe porque entre pedirlo y usarlo pasa tiempo, y un token que caduca en vuelo se ve
 * exactamente igual que una credencial mala: un 401.
 *
 ...
 * <b>no</b> puede pasar es que un fallo de transporte mate un evento.
 *
 * <p>Copiado de {@code rentas} con sus motivos (ver el {@code package-info}).
 */
public class TokenDeServicioDeKeycloak implements CredencialDeServicio {
    ...
    public String cabecera() {
        // Sin configurar, cadena vacia y no una excepcion: el cliente HTTP ya sabe decir «este
        // consumidor no manda ninguna», y con el nombre de la propiedad que falta. Lanzar aqui
        ...
        synchronized (this) {
            ...
            // vigencia que el emisor no dijo cuesta un 401 a mitad de una vuelta.
