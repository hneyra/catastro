...
public class CacheDeSnapshotsJdbc extends RepositorioJdbc implements CacheDeSnapshots {
    ...
    /**
     * El anexo vehicular <b>no se cachea aqui</b> (P5C).
     *
     * <p>`normativa` sirve el snapshot entero, y de sus cuatro cuadros este sistema solo usa dos:
     * los valores unitarios de edificacion y la depreciacion. El valor referencial de un vehiculo
     * es del calculo del impuesto vehicular, que vive en `rentas`, y `V2` de este repositorio no
     * crea `normativa_valor_referencial` — de modo que insertarlo aqui no seria un desperdicio de
     * 54 000 filas sino un error en la primera descarga.
     *
     * <p>Se deja el metodo con el cuerpo vacio y no se borra la llamada, para que quien lea
     * `guardar` vea las cuatro mitades del snapshot y esta diga por que no escribe.
     */
    private void insertarValoresReferenciales(SnapshotDeNormativa snapshot) {
        // Sin cuerpo a proposito: ver el javadoc.
    }
}
