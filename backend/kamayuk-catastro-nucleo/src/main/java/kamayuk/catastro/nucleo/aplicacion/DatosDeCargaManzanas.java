 ...
 */
@ConfigurationProperties("kamayuk.carga-manzanas")
    ...
    public DatosDeCargaManzanas {
        if (municipalidadId < 1) {
            throw new IllegalArgumentException(
                    "Falta kamayuk.carga-manzanas.municipalidad-id, o no es un identificador valido");
        }
        if (archivo == null || archivo.isBlank()) {
            throw new IllegalArgumentException(
                    "Falta kamayuk.carga-manzanas.archivo, que no tiene valor por omision");
        }
