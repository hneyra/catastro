 ...
 * http(s)://} para poder comprobar la entrega ejecutandola. Aqui se midio antes de copiarla: el
 * canal que los ambientes declaran es un CORREO —{@code KAMAYUK_CANAL_DE_OPERACION=operaciones@…}
 * en el {@code .env} de la plataforma, y {@code kamayuk:canalDeOperacion} en los dos stacks—, y es
 * lo que el descriptor pone en {@code KAMAYUK_IDENTIDAD_CANAL} desde {@code e.operacion.canal}.
 * Exigir http aqui haria que la implantacion —que termina con una pasada del consumidor— no
 * arrancara en ningun ambiente. Asi que el aviso <b>siempre</b> se registra con nivel ERROR con el
 * responsable y su canal dentro, y <b>ademas</b> se entrega con un {@code POST} cuando el canal es
 * una direccion http(s). Lo que cuesta queda dicho: con un correo, la unica constancia es esa
 * linea, y la observabilidad del proyecto (INF-11) alerta sobre ERROR.
 */
