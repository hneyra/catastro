 ...
 * kamayuk.catastro.nucleo.aplicacion.PublicacionDelPadron}— ya recibe su {@code Clock}, asi que con
 * {@code now()} la corrida y sus eventos quedaban fechados por dos relojes distintos, y eso no se
 * ve hasta que no coinciden. El propio: esa hora <b>sale publicada</b> —es el {@code emitidoEn} de
 * {@code EventoResource}— y con ella {@code docs/50-api/eventos/lote-de-eventos.json}, el lote que
 * {@code rentas} lee, se reescribia en cada corrida del banco de pruebas. Su unico diff eran los
 * cinco {@code emitidoEn}, de modo que {@code git status} salia sucio siempre y un cambio de verdad
 * en la forma del evento —que es justo lo que ese archivo existe para enseñar— llegaba mezclado con
 * cinco instantes que no significan nada.
 */
@Repository
