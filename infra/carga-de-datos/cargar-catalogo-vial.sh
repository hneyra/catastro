...
SUFIJO=$(date +%s)
RECURSO="kamayuk-${AMBIENTE}-carga-vial-${SUFIJO}"
            ...
            - name: KAMAYUK_DB_CLAVE
              valueFrom:
                secretKeyRef:
                  ...
                  key: clave-app
            - name: KAMAYUK_CARGAVIAL_MUNICIPALIDADID
              value: "$MUNICIPALIDAD_ID"
            - name: KAMAYUK_CARGAVIAL_ARCHIVO
              value: /datos/vias.csv
            - name: KAMAYUK_CARGAVIAL_USUARIODELPROCESO
              value: carga-vial
            - name: KAMAYUK_CARGAVIAL_OBSERVACION
              value: "$OBSERVACION"
          volumeMounts:
            - name: datos
              mountPath: /datos
